		...
		<div class="row">
			<nav class="col-md-3 col-lg-2 d-md-block bg-light sidebar p-3">
				...
				<ul>
					...
					<li class="nav-item"><a href="/inquiry_list" class="nav-link">1:1
							문의</a></li>
			...
			<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
				...
				<div class="search-bar d-flex justify-content-between mb-4">
					<div class="search-container">
						<form action="/inquiry_search" method="get">
				...
				<div class="register-link">
					<p>
						<a href="/inquiry_list" class="btn-custom">정보 새로고침</a>
					</p>
				</div>
							...
							<c:otherwise>
								<c:forEach items="${list}" var="k" varStatus="c">
									<tr>
										...
										<td>${k.subject}</td>
										<td>${k.category }</td>
										<td><script>document.write(maskUserId('${k.user_id}'))</script></td>
										<td>${k.write_date }</td>
										<c:choose>
											<c:when test="${k.answer == null}">
												<td style="color: black;">답변 전</td>
											</c:when>
											<c:when test="${k.answer != null}">
										...
										<td><a
											href="/inquiry_detail?u_idx=${k.u_idx }"
				...
				<div class="d-flex justify-content-center">
					<c:if test="${currentPage > 1}">
						<a href="/inquiry_list?page=${currentPage - 1}"
