...
public class LoginController {
    ...
    public ModelAndView join1Form() {
    	ModelAndView mv = new ModelAndView("saupjoin");
 ...
 // 로그아웃 처리
    ...
    public String myPage(HttpSession session) {
        ...
        if ("ok".equals(loginStatus)) {
            return "mypage";  // 로그인 상태일 때만 mypage.jsp로 이동
        } else {
            return "redirect:/login";  // 로그인 상태가 아닐 경우 로그인 페이지로 이동
        }
    }
 ...
 // 아이디 찾기 처리
    ...
    public ModelAndView findUserPassword(MemberVO memberVO, HttpSession session) {
        ...
        try {
            ...
            if (foundUser != null) {
                mv.setViewName("find_password_change");
