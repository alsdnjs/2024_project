                	...
                	<c:choose>
                		...
                		<c:otherwise>
                			<c:forEach var="k" items="${list}" varStatus="c">
                				...
                				<strong>${k.category}</strong><br>
                				<a href="/inquiryDetail?u_idx=${k.u_idx}&cPage=${paging.nowPage}">${k.subject}</a>
                				&nbsp;&nbsp;&nbsp;${k.write_date}</p>
			                        ...
			                        <div class="accordion-item">
			                            <h2 class="accordion-header" id="heading${c.index}">
			                                <button class="accordion-button collapsed" type="button" 
			                                	data-bs-toggle="collapse" data-bs-target="#collapse${c.index}" 
			                                		aria-expanded="false" aria-controls="collapse${c.index}">
			                                    답변 보기
			                                </button>
			                            </h2>
			                            
			                            <div id="collapse${c.index}" class="accordion-collapse collapse" 
			                            	aria-labelledby="heading${c.index}" data-bs-parent="#accordionExample${c.index}">
			                                <div class="accordion-body">
			                                    판매자 답변: 
			                                    <c:choose>
													<c:when test="${empty inquiry.answer}">
														&nbsp;<a>아직 답변이 등록되지 않았습니다.</a>
													</c:when>
													<c:otherwise>
														<a>${inquiry.answer}</a>
													</c:otherwise>
				...
				<!-- 페이지기법 -->
						<div class="paging">
						   ...
						   <c:choose>
						   		...
						   		<c:otherwise>
						   			<p><a href="/myInquiry?cPage=${paging.beginBlock - paging.pagePerBlock}"><<</a></p>
						   		</c:otherwise>
						   </c:choose>
						    <!-- 블록안에 들어간 페이지번호들 -->
